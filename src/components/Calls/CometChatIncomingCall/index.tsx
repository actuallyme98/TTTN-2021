...
import { incomingCallAlert } from '../../../assets/audio';
import { logger } from '../../../utils/common';
export default (props) => {
  let callAlertManager: CallAlertManager | null = null;
      ...
      if (activeCall) {
        CometChat.rejectCall(call.sessionId, CometChat.CALL_STATUS.BUSY)
          .then((rejectedCall) => {
            ...
            props.actionGenerated(actions.REJECTED_INCOMING_CALL, call, rejectedCall);
          })
          .catch((error) => {
  ...
  const rejectCall = () => {
    try {
      ...
      CometChatManager.rejectCall(incomingCall.sessionId, CometChat.CALL_STATUS.REJECTED)
        .then((rejectedCall) => {
          props.actionGenerated(actions.REJECTED_INCOMING_CALL, incomingCall, rejectedCall);
          setIncomingCall(null);
        })
        .catch((error) => {
          props.actionGenerated(actions.CALL_ERROR, error);
